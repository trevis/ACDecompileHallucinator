...
{
    private static readonly Regex BaseclassPattern = new Regex(@"^baseclass_[0-9a-fA-F]+$", RegexOptions.Compiled);

    public static void ParseNameAndInheritance(StructTypeModel structModel, string source)
    {
        // Find the line that contains the struct keyword, since it might not start with "struct"
        var lines = source.Split(new[] { '\r', '\n' }, StringSplitOptions.RemoveEmptyEntries);
        string defLine = "";

        foreach (var line in lines)
        {
            if (System.Text.RegularExpressions.Regex.IsMatch(line, @"\bstruct\b"))
            {
                defLine = line;
                break;
            }
        ...
        {
            Console.WriteLine($"Warning: Could not find struct definition line in source:");
        ...
        // Check if the original definition line starts with 'const struct' or 'volatile struct' before cleaning
        string originalDefLine = defLine.Trim();
        // Handle cases like "const struct __cppobj TypeName", "volatile struct TypeName", etc.
        if (originalDefLine.StartsWith("const struct"))
        {
            // Remove the "const " prefix for further processing
            defLine = originalDefLine.Substring(6).Trim(); // Remove "const "
        }
        else if (originalDefLine.StartsWith("volatile struct"))
        {
            // Remove the "volatile " prefix for further processing
            defLine = originalDefLine.Substring(9).Trim(); // Remove "volatile "
            structModel.IsVolatile = true; // Mark the struct as volatile
        }

        // Extract alignment info before cleaning the line
        var (hasAlignment, alignment) = ParsingUtilities.ExtractAlignmentInfo(defLine);
        if (hasAlignment)
        {
            structModel.Alignment = alignment;
        }

        defLine = ParsingUtilities.CleanDefinitionLine(defLine, "struct");

        defLine = TypeParser.EatType(defLine, out var typeName);

        var parsedType = TypeParser.ParseType(typeName);
        structModel.Name = parsedType.BaseName;
        structModel.Namespace = parsedType.Namespace;
        ...
        structModel.TemplateArguments.AddRange(templateArgs);
        ...
        foreach (var baseType in baseTypes)
        {
            ...
            structModel.BaseTypes.Add(parsedBaseType.BaseName);
        }
    }

    public static void ParseMembers(StructTypeModel structModel, List<string> lines, int structStartIndex,
        string? file = null)
    {
        // Find the opening brace of the struct
        int braceStart = -1;
        for (int i = structStartIndex; i < lines.Count; i++)
        {
            var line = lines[i];
            if (line.Contains('{'))
            {
        ...
        for (int i = braceStart + 1; i < braceEnd; i++)
        {
            ...
            int? memberLineNumber = structModel.LineNumber.HasValue
                ? structModel.LineNumber.Value + (i - structStartIndex)
                : (int?)null;
            ...
            if (member != null)
            {
                ...
                {
                    Console.WriteLine(
                        $"Warning: Struct member has empty name in '{structModel.FullyQualifiedName}' ({file}:{memberLineNumber}): {line}");
                }
                ...
                {
                    Console.WriteLine(
                        $"Warning: Struct member name contains spaces in '{structModel.FullyQualifiedName}' ({file}:{memberLineNumber}): '{member.Name}' from: {line}");
                }

                if (string.IsNullOrWhiteSpace(member.TypeString))
                {
                    Console.WriteLine(
                        $"Warning: Struct member has empty type in '{structModel.FullyQualifiedName}' ({file}:{memberLineNumber}): {line}");
                }
                ...
                // Check if this is a baseclass member (format: baseclass_<hex_offset>)
                if (IsBaseclassMember(member.Name))
                {
                    // Add the type to BaseTypes instead of Members
                    structModel.BaseTypes.Add(member.TypeString);
                }
                else
                {
                    structModel.Members.Add(member);
                }
            }
            else
            {
                ...
                Console.WriteLine(
                    $"Warning: Failed to parse struct member in '{structModel.FullyQualifiedName}' ({file}:{memberLineNumber}): {line}");
            }
        }
    }
    ...
    private static bool IsBaseclassMember(string memberName)
    {
        return BaseclassPattern.IsMatch(memberName);
    }
}
