...
public static class DefaultHierarchyRules
{
    public static List<IHierarchyRule> GetDefaultRules()
    {
        return new List<IHierarchyRule>
        {
            // templated types
            new FqnMatchRule(
                "^(LongHash|InterfacePtr|PS|NILI|UI64|SmartArray|DArray|SimpleDArray|List|Hash|SmartBuffer|Intrusive|AutoGrow|LongNI|PackableL|DLList|PackableHa|RefCountIUnknown)",
                "Lib/Templates/", NamespaceBehavior.StripAll, 1),
            ...
            new FqnMatchRule("Connection|Packet|^Blob|^(?!.*Scene).*Net.*$|CLink", "Net/", NamespaceBehavior.KeepAll,
                1),
            ...
            // Input
            new FqnMatchRule(
                "^(C?Input|Device|CommandInterp|I?Keystone|ActionState|ArgumentParser|ATTNAMESSTRUCT|CommandLineArg)",
                "Input/", NamespaceBehavior.KeepAll, 1),
            ...
            new FqnMatchRule("(Render|DepthTest|Vertex|Blit|Blend|Graphics|CullMode)", "Rendering/",
                NamespaceBehavior.KeepAll, 1),
            ...
            new FqnMatchRule("^(DBObjGrabber|DATFILE_|Archive|DBObj|TransientArchive)", "Dats/",
                NamespaceBehavior.StripAll, 0),
