...
public class StageResultRepository : IStageResultRepository
{
    ...
    public async Task<HashSet<(EntityType, string)>> GetCompletedEntityNamesAsync(string stageName)
    {
        var results = await _db.StageResults
            .Where(r => r.StageName == stageName && r.Status == StageResultStatus.Success)
            .Select(r => new { r.EntityType, r.FullyQualifiedName })
            .ToListAsync();

        return results.Select(r => (r.EntityType, r.FullyQualifiedName)).ToHashSet();
    }
    ...
    public async Task SaveResultAsync(StageResult result)
    {
        var existing = await _db.StageResults
            .FirstOrDefaultAsync(r =>
                ...
                r.FullyQualifiedName == result.FullyQualifiedName);
    ...
    public async Task<StageResult?> GetSuccessfulResultAsync(
        string stageName, EntityType entityType, string fullyQualifiedName)
    {
    ...
    public async Task<List<StageResult>> GetResultsWithLogsAsync(EntityType entityType, string fullyQualifiedName)
    {
        return await _db.StageResults
            .Include(r => r.RequestLogs)
            .Where(r => r.EntityType == entityType && r.FullyQualifiedName == fullyQualifiedName)
